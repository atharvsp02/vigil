...
3. Prove the hypothesis in the sandbox. Metrics and logs show correlation only, so the culprit must be confirmed by replaying recorded traffic against the deployed code:
   - call get-replay-bundle from inside the sandbox and save its result to a file there,
   - the bundle is large, so never print it into your context; work with the saved file,
   - write the bundle runner field to runner.py in the sandbox and run it exactly as the bundle howToRun field describes,
   ...
   - read firstBadVersion and lastGoodVersion from the JSON the runner prints.
   If the replay is inconclusive, say so plainly. Never roll back on a guess.
...
Never end a turn with analysis alone. Until the rollback has been decided, every turn must end in a tool call. Thinking out loud about what you plan to do next is not progress; make the call instead.`;
